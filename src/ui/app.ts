...
function renderProofCourse(): void {
  ...
  function render(): void {
    ...
    root.innerHTML = `<div class="app-shell">
      ...
      <main class="workspace">
        ...
        <section class="proof-panel">
          <div class="theorem-header"><div><div class="section-label">Chapter ${chapter.number} · Exercise ${exercise.number}</div><h1>${exercise.title}</h1><p class="theorem-statement"><code>${exercise.statement}</code></p></div><button id="restart-button" class="next-button" type="button" ${state ? "" : "disabled"}>Restart proof</button></div>
    ...
    root.querySelector<HTMLButtonElement>("#restart-button")?.addEventListener("click", () => selectExercise(exercise));
